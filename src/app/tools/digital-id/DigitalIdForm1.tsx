...
export default function DigitalIDForm({ lang }: { lang: LangType }) {
  ...
  const [spaceType, setSpaceType] = useState<
    "personal" | "organization" | "developer" | "startup" | "business"
  >("personal");

  const [spaceSubType, setSpaceSubType] = useState("");
  ...
  const handleActivate = async () => {
    if (!legalTerms || (spaceType === "organization" && !legalAuth)) {
      alert(t.error_legal);
      return;
    }
  ...
  const downloadQR = () => {
    ...
    link.download = `retailbox-space-${spaceType}.png`;
            ...
            <div className="bg-white dark:bg-slate-900 p-6 md:p-10 rounded-[3rem] shadow-xl border border-gray-100 dark:border-slate-800 space-y-8">
              ...
              <div className="space-y-4">
                ...
                >
                  ...
                  {isSlugAvailable === false && (
                    <X className="text-red-500 ml-2" size={18} />
                  )}
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                {links.map((link, i) => {
                  const networkConfig =
                    SOCIAL_CONFIG[link.network as keyof typeof SOCIAL_CONFIG];
                    ...
                    >
                      <div className="flex gap-2">
                        <select
                          value={link.network}
                          onChange={(e) =>
                            updateLink(i, "network", e.target.value)
                          }
                          className="
                            flex-1 p-3
                      ...
                      <input
                        value={link.handle}
                        onChange={(e) =>
                          updateLink(i, "handle", e.target.value)
                        }
                        placeholder={networkConfig.ph || t.ph_handle}
                        className="
                          w-full p-4
                          bg-white dark:bg-slate-800
                          border-none rounded-xl
                ...
                {spaceType === "organization" && (
                  <label className="flex items-start gap-3 cursor-pointer group">
                    <input
                      type="checkbox"
                      checked={legalAuth}
                      onChange={(e) => setLegalAuth(e.target.checked)}
                      className="mt-1 accent-indigo-600 w-4 h-4"
                    />
                ...
                <QRCodeCanvas
                  ...
                  imageSettings={
                    logo
                      ...
                      : {
                          src: getQrIcon(ICON_PATHS.users, fgColor),
                          height: 40,
                          width: 40,
                          excavate: true,
                        }
