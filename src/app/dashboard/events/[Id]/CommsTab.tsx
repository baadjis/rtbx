...
export default function CommsTab({ 
  ...
  lang, 
  t,
  // Props partagées passées par le parent
  automationType, setAutomationType,
  orgLogo, setOrgLogo,
  orgName, setOrgName,
  sponsors, setSponsors,
  usefulInfo, setUsefulInfo,
  themeColor,
  handleSave,
  loading
}: any) {
  ...
  // Logique d'upload d'image (Logo ou Sponsors)
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in slide-in-from-right-8 duration-500 text-gray-900 dark:text-white">
      ...
      {/* --- COLONNE GAUCHE : DIFFUSION & AUTOMATISATION --- */}
      <div className="space-y-6">

        {/* 1. QR CODE DE DIFFUSION (FLYER) */}
        <EventQRCode 
          eventId={event.id}
        ...
        {/* 2. STRATÉGIE D'ENVOI AUTOMATISÉ */}
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] border border-gray-100 dark:border-slate-800 shadow-sm transition-colors">
          <h3 className="text-xl font-black mb-6 flex items-center gap-3">
            <Zap className="text-indigo-600" /> {t.automation_title}
          </h3>
          <div className="space-y-3">
            {[
                { id: 'manual', label: t.opt_manual, icon: MousePointer2 }, 
                { id: 'immediate', label: t.opt_immediate, icon: Zap }, 
                { id: 'scheduled', label: t.opt_scheduled, icon: Clock }
            ].map((opt) => (
              <button 
                key={opt.id} 
                onClick={() => setAutomationType(opt.id)} 
                className={`w-full flex items-center justify-between p-5 rounded-[2rem] border-2 transition-all bg-transparent cursor-pointer ${
                    automationType === opt.id 
                    ? 'border-indigo-600 bg-indigo-50/50 dark:bg-indigo-900/20' 
                    : 'border-gray-50 dark:border-slate-800 opacity-60'
                }`}
              >
                <div className="flex items-center gap-4">
                  <div className="p-2 bg-white dark:bg-slate-800 rounded-lg shadow-sm text-gray-400">
                    <opt.icon size={20} />
                  </div>
      ...
      {/* --- COLONNE DROITE : BRANDING & MAILING --- */}
      <div className="space-y-6">
        <div className="bg-white dark:bg-slate-900 p-8 md:p-12 rounded-[3.5rem] border border-gray-100 dark:border-slate-800 shadow-xl space-y-8">
            <h3 className="text-2xl font-black mb-2 flex items-center gap-3 uppercase tracking-tight">
                <Settings2 className="text-indigo-600" /> {t.comm_title}
            </h3>
            ...
            <div className="grid grid-cols-4 gap-4">
                {sponsors.map((s:any, i:any) => (
                    <div key={i} className="relative aspect-square bg-gray-50 dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 overflow-hidden group shadow-sm">
                        ...
                        <button onClick={() => setSponsors(sponsors.filter((sp:any, idx:any) => idx !== i))} className="absolute inset-0 bg-red-600/80 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity border-none cursor-pointer"><X size={20} /></button>
                    </div>
                ))}
                {sponsors.length < 4 && (
