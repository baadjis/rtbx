...
import { 
  ...
  ChevronRight, Wifi, ArrowRight, MapPin
} from 'lucide-react';
...
export default async function DashboardPage() {
  ...
  // 2. Fonction utilitaire pour l'icône dynamique du business
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-10 py-8 md:py-12 space-y-10 transition-colors duration-300">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-[2.5rem] border border-gray-100 dark:border-slate-800 shadow-sm transition-all group">
          ...
          <p className="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest">{t.average_rating || (lang === 'fr' ? 'Note Globale' : 'Global Rating')}</p>
          <h3 className="text-4xl font-black text-gray-900 dark:text-white mt-1">{businesses.length > 0 ? "4.8" : "--"}<span className="text-sm text-gray-400">/5</span></h3>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-[2.5rem] border border-gray-100 dark:border-slate-800 shadow-sm transition-all group">
          ...
          <p className="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest">{t.linked_businesses || (lang === 'fr' ? 'Commerces' : 'Businesses')}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[3rem] border border-gray-100 dark:border-slate-800 shadow-xl p-8 flex flex-col">
            <div className="flex justify-between items-center mb-8">
                <h2 className="text-2xl font-black text-gray-900 dark:text-white tracking-tight">{t.my_retail_spaces || (lang === 'fr' ? 'Mes Commerces' : 'My Businesses')}</h2>
                <Link href="/dashboard/businesses" className="text-indigo-600 dark:text-indigo-400 font-black text-[10px] uppercase tracking-widest hover:underline no-underline">
                    {t.view_all_biz || (lang === 'fr' ? 'Voir tout' : 'View all')}
                </Link>
            </div>
            ...
            <div className="mt-8 pt-6 border-t border-gray-50 dark:border-slate-800 text-center">
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-[0.2em]">Retail Manager • {lang.toUpperCase()}</p>
            </div>
        </div>
